...
if (!empty($_REQUEST['username']) && !empty($_REQUEST['password'])) {
	$sql	= sprintf("SELECT * FROM %sCubeCart_admin_users WHERE username = %s AND password = MD5(%s) AND isSuper = '1'", $glob['dbprefix'], $db->mySQLsafe($_REQUEST['username']), $db->mySQLsafe($_REQUEST['password']));
	if ($db->numrows($sql) == 1) {
		## Authorised
		## Generate XML data
		...
		$ordersql	= sprintf("SELECT cart_order_id, prod_total, time FROM %sCubeCart_order_sum WHERE status = 1 ORDER BY time DESC", $glob['dbprefix']);
		$xml[] = '<orders>';
		...
		$reviewsql	= sprintf("SELECT time FROM %sCubeCart_reviews WHERE approved = '0'", $glob['dbprefix']);
		$xml[] = '<reviews>';
		...
		if ($config['stock_warn_type'] == 1) {
			$stocksql	= sprintf("SELECT productId, stock_level FROM %sCubeCart_inventory WHERE useStockLevel = '1' AND stock_level < stockWarn ORDER BY stock_level ASC", $glob['dbprefix']);
		} else {
			if (!isset($config['stock_warn_level'])) $config['stock_warn_level'] = 5;
			$stocksql = sprintf("SELECT productId, stock_level FROM %sCubeCart_inventory WHERE useStockLevel = 1 AND stock_level <= %d ORDER BY stock_level ASC", $glob['dbprefix'], $config['stock_warn_level']);
		}
