...
if (version_compare(PHP_VERSION, '5.1.0', '<')) {
	die('You need to upgrade to PHP Version 5.1.0 or better to use CubeCart. You are currently running PHP Version '.PHP_VERSION);
}
	...
	public function clean_data(&$data) {
		...
		if (is_array($data)) {
			foreach ($data as $key => $val) {
				...
				if (empty($val) || in_array($key, $skipKeys)) {
					continue;
