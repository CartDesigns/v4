...
if ((bool)$_GET['process']) {
	...
	$_POST['iso_state'] 	= ($ISOstate==true) ? $ISOstate[0]['abbrev'] : trim($_POST['state']);
	$_POST['iso_country'] 	= $countryIso;
	
	# Card data has to go into session variables due to 3D secure pages
	/*
	$_SESSION['ppcarddata']['firstName'] 		= urlencode(trim($_POST['firstName']));
	$_SESSION['ppcarddata']['lastName'] 		= urlencode(trim($_POST['lastName']));
	$_SESSION['ppcarddata']['addr1']	 		= urlencode(trim($_POST['addr1']));
	$_SESSION['ppcarddata']['addr2']			= urlencode(trim($_POST['addr2']));
	$_SESSION['ppcarddata']['city'] 			= urlencode(trim($_POST['city']));
	$_SESSION['ppcarddata']['state'] 			= urlencode($processState);
	$_SESSION['ppcarddata']['postalCode']		= urlencode(trim($_POST['postalCode']));
	$_SESSION['ppcarddata']['country']		= urlencode($countryIso); // specified in form.inc.php
	*/
	...
	if (isset($_POST['cardNumber'])) {
		...
		if ((bool)$module['3ds_status']) {
			...
			$centinel_url	= ((bool)$module['gateway']) ? 'https://paypal.cardinalcommerce.com/maps/txns.asp':'https://centineltest.cardinalcommerce.com/maps/txns.asp';
			$centinel->sendHttp($centinel_url, 5, 10);
		
			if ($centinel->getValue('Enrolled') == 'Y' && $centinel->getValue('ErrorDesc') == 0) {
				## Enrolled, get data, and display iframe content				
				$_SESSION['centinel_data']	= array(
					...
					'TermUrl'			=> $GLOBALS['storeURL'].'/index.php?_g=co&_a=step3&process=1&cart_order_id='.$orderSum['cart_order_id'],
				);
				$display_3ds	= true;
			} else {
	...
	if ((bool)$module['3ds_status'] && isset($_POST['PaRes'])) {
		...
		$centinel->add('TransactionPwd', $module['3ds_password']);
		$centinel->add('TransactionType', 'C');
		$centinel->add('TransactionId', $_SESSION['centinel_data']['TransactionId']);
		...
		if ($centinel->getValue('ErrorNo') == 0 && in_array($centinel->getValue('PAResStatus'), array('Y', 'A')) && $centinel->getValue('SignatureVerification') == 'Y') {
			$response	= array(
				'AUTHSTATUS3DS'		=> $centinel->getValue('PAResStatus'),
				'MPIVENDOR3DS'		=> $_SESSION['centinel_data']['Enrolled'],
				'CAVV'				=> $centinel->getValue('Cavv'),
				'ECI'				=> $centinel->getValue('EciFlag'),
				'XID'				=> $centinel->getValue('Xid'),
			);
			$_SESSION['centinel_data']	= (array)$response;
			$process_payment	= true;
			...
			$_SESSION['centinel_data']['error']	= 'Your financial institution has indicated that it could not successfully authenticate this transaction. To protect against unauthorized use, this card cannot be used to complete your purchase. You may complete the purchase by selecting another form of payment.';
		}
	}
	...
	if (!$display_3ds && $process_payment) {
		...
		switch (strtoupper($ack)) {
			case 'SUCCESS':
				...
				unset($_SESSION['centinel_data'], $_SESSION['cardinfo']);
				break;
			case 'SUCCESSWITHWARNING':
				...
				## Update Gateway Name for order
$db->update($glob['dbprefix']."CubeCart_order_sum",array('gateway' => $transData['gateway']),"cart_order_id = ".$db->MySQLSafe($_GET["cart_order_id"]));
				## Redirect
			...
			default:
				...
				$jumpTo					= 'index.php?_g=co&_a=step3&wpp=true&cart_order_id='.$orderSum['cart_order_id'];
				$_SESSION['centinel_data']['error'] = $errorMsg['customer'];
		}
		$order->storeTrans($transData);
	}
	if (isset($jumpTo) && !empty($jumpTo)) {
		if ((bool)$iframe_redirect) {
			echo sprintf('<script type="text/javascript">parent.location=\'%s\';</script>', $jumpTo);
...
if ((bool)$display_3ds) {
	...
	if (isset($_SESSION['centinel_data']['error'])) {
		$errorMsg['customer']	= $_SESSION['centinel_data']['error'];
		unset($_SESSION['centinel_data']);
	}
	...
	if ($config['defaultCurrency'] == 'USD' && $module['mode']=='US' || $module['mode']=='USDPO') {
		...
		if(!isset($module['amex']) || $module['amex']) {
			$cards = array_merge($cards,array("Amex" => "American Express"));
		}
	...
	if ((bool)$module['3ds_status']) {
		$formTemplate->parse('form.3ds');
		$formTemplate->parse('form.3ds_logo');
	}
