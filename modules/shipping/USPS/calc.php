...
function USPS() {
	...
	if ($module['status'] == 1) {
		...
		if (empty($basket['delInf']['postcode'])) {
		...
		foreach ($module as $moduleKey => $moduleValue) {
			if (preg_match('#service\_#i', $moduleKey) && $moduleValue) {
				...
				$moduleKey = str_replace(array('service','EMS'), '', $moduleKey);
					...
					if (!$overWeight) {
						...
						if (is_array($rates)) {
							...
							foreach ($rates as $key => $value) {
							    ...
							    $key = str_replace(array('&lt;sup&gt;&amp;trade;&lt;/sup&gt;','&lt;sup&gt;&amp;reg;&lt;/sup&gt;'),'',$key);

								$compareKey = preg_replace('/[^a-zA-Z0-9s]/', '', $key);


								if(($usps->api == 'IntlRate' && $module[$compareKey]) || $usps->api == 'RateV3') {
									$sum = $value;
