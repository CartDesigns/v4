...
*/
	...
	$print_order_form = new XTemplate('modules/gateway/Print_Order_Form/orderForm.tpl','',null,'main',true,true);
	...
	for($i = 0, $maxi = count($orderInv);$i < $maxi;++$i)
	{
	...
	if ($config_tax_mod['status'])
	{
		...
		for ($i=1; $i<3; ++$i)
		{
			if (!empty($orderSum['tax'.($i+1).'_disp']))
			{
	...
	if(!empty($module['notes'])){
		$print_order_form->assign('VAL_CUST_NOTES',nl2br($module['notes']));
