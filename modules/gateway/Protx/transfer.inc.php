...
function fixedVars() {
	...
	global $module, $orderSum, $config, $db;
	
	// This makes a VERY random 40 character unique transaction code. Chances of clash almost impossible.
	$VendorTxCode = md5(randomPass(3).time().rand(0,32000)).randomPass(8);
	...
	*/
	...
	if($invCountry=="US") {
		$full_state = $db->select("SELECT `abbrev` FROM `".$config['dbprefix']."CubeCart_iso_counties` WHERE `name` = '".$orderSum['county']."'");
		$cryptVars.="&BillingState=".$full_state[0]['abbrev']; // optional
	}
	...
	if($delCountry=="US") {
		$full_state = $db->select("SELECT `abbrev` FROM `".$config['dbprefix']."CubeCart_iso_counties` WHERE `name` = '".$orderSum['county_d']."'");
		$cryptVars.="&DeliveryState=".$full_state[0]['abbrev']; // optional
	}
