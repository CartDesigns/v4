...
function fixedVars() {
	...
	$hiddenVars = "<input type='hidden' name='cmd' value='_xclick' />
				...
				<input type='hidden' name='night_phone_a' value='".$orderSum['phone']."' />
				<input type='hidden' name='add' value='1' />
				<input type='hidden' name='rm' value='2' />
				<input type='hidden' name='no_note' value='1' />";

				if($config['defaultCurrency']=="CAD") {
					$hiddenVars .= "<input type='hidden' name='bn' value='CubeCart_Cart_ST_CA'>";
				} else {
					$hiddenVars .= "<input type='hidden' name='bn' value='CubeCart_Cart_ST'>";
				}
