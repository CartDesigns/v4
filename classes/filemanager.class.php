...
class FileManager {
	...
	public function showFileList($type, $page = 0, $perPage = 50) {
		...
		$sql	= sprintf("SELECT * FROM %sCubeCart_filemanager WHERE type = '%d' AND disabled = '0' ORDER BY `filepath` ASC %s", $this->_config['dbprefix'], $type, $limit);
	...
	public function buildDatabase($purge = false) {
		...
		if ($fileArray) {
			if ($purge) {
				$this->_db->truncate($this->_config['dbprefix'].'CubeCart_filemanager');
			} else {
				$dupes	= sprintf("SELECT filepath FROM %sCubeCart_filemanager WHERE 1 ORDER BY filename ASC;", $this->_config['dbprefix']);
			...
			foreach ($fileArray as $key => $file) {
				...
				#	$checksql	= sprintf("SELECT COUNT(`file_id`) as Count FROM %sCubeCart_filemanager WHERE md5hash = %s AND filepath LIKE '%s%%';", $this->_config['dbprefix'], $record['md5hash'], $target_dir);
	...
	public function deleteFile($file_id) {
		$sql	= sprintf("SELECT `filepath` FROM `%sCubeCart_filemanager` WHERE `file_id` = %d", $this->_config['dbprefix'], $file_id);
		...
		if ($result) {
			...
			$sql = sprintf('DELETE FROM `%sCubeCart_filemanager` WHERE `file_id` = %d', $this->_config['dbprefix'], $file_id);
			$this->_db->misc($sql);
			## Delete from image idx table
			$idx_path = str_replace("images/uploads/","",$result[0]['filepath']);
			## Update image counts
			$sql	= sprintf("SELECT `productId` FROM `%sCubeCart_img_idx` WHERE `img` = '%s'", $this->_config['dbprefix'], $idx_path);
			$affectedProducts = $this->_db->select($sql);
			if($affectedProducts){
				foreach($affectedProducts as $row){
					## Set product count for product to n -1
					$sql = sprintf('UPDATE `%sCubeCart_inventory` SET `noImages` = `noImages` -1 WHERE `productId` = %d', $this->_config['dbprefix'], $row['productId']);
					$this->_db->misc($sql);
				}
			}
			## Remove associated file name from inventory table
			$sql = sprintf("UPDATE `%sCubeCart_inventory` SET `image` = '' WHERE `image` = '%s'", $this->_config['dbprefix'], $idx_path);
			$this->_db->misc($sql);
			## Remove index from table
			$sql = sprintf("DELETE FROM `%sCubeCart_img_idx` WHERE `img` = '%s'", $this->_config['dbprefix'], $idx_path);
			$this->_db->misc($sql);
			return true;
		}
		return false;
