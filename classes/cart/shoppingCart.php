...
class cart {
	...
	function add($productId, $quantity = 1, $options) {
	...
	function sqlValue() {
		...
		$query		= sprintf('SELECT basket FROM %sCubeCart_sessions WHERE `sessId`=%s', $glob['dbprefix'], $db->mySQLSafe($GLOBALS[CC_SESSION_NAME]));
	...
	function getOptions($productKey) {
		...
		return html_entity_decode($options[1],ENT_QUOTES,'UTF-8');
	}
	...
	*/
	...
	function addCoupon($code) {
		...
		if ($coupon) {
			...
			} else if (!empty($coupon[0]['expires']) && (strtotime($coupon[0]['expires']) < $timeNow)) {
				## coupon expired
				$this->setVar(2, 'codeResult');
			} else {
				## success
	...
	function addByCode($code) {
		global $glob, $config, $db;
		$result = $db->select('SELECT productId, stock_level FROM '.$glob['dbprefix'].'CubeCart_inventory WHERE `productCode` = '.$db->mySQLSafe($code));
		if ($result) {
