...
function generateCategoryUrl($catid) {
	...
	if (is_numeric($catid)) {
		...
		if (!$cache->cacheStatus) {
			$query = sprintf("SELECT `cat_name`, `cat_id`, `cat_father_id`, `seo_custom_url` FROM %sCubeCart_category WHERE `cat_id`=%d", $glob['dbprefix'], $catid);
...
function generateProductUrl($productid) {
	...
	if (!$cache->cacheStatus) {
		$query			= sprintf("SELECT I.productId, I.name, I.cat_id, I.seo_custom_url, C.cat_name, C.cat_father_id FROM %1\$sCubeCart_inventory AS I INNER JOIN %1\$sCubeCart_category AS C ON I.cat_id = C.cat_id WHERE I.productId='%2\$d' AND I.cat_id > 0", $glob['dbprefix'], $productid);
...
function generateDocumentUrl($docid) {
	...
	return $glob['storeURL'].'/'.strtolower(generateSafeUrls($doc).'/' . $sefpre . $docid . $ext);
}
