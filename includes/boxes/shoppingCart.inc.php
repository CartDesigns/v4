...
if (!$cc_session->user_is_search_engine()) { // || !$config['sef']) {
	...
	if (isset($_POST['add']) && $_POST['add']>0) {
		...
		if ($config['add_to_basket_act']) {
			...
			if ($cc_session->ccUserData['customer_id']>0) {
				...
				httpredir($GLOBALS['rootRel']."index.php?_g=co&_a=cart");
			}
		} else {
	...
	} else if (isset($_POST['gc']['cert']) && $_POST['gc']['cert']) {
	...
	if (is_array($basket['conts']) && !empty($basket['conts'])) {
		foreach ($basket['conts'] as $key => $value) {
			if (isset($basket['conts'][$key]['custom']) && $basket['conts'][$key]['custom']) {
				...
				$price	= (!salePrice($product[0]['price'], $product[0]['sale_price'])) ? $price = $product[0]['price'] : salePrice($product[0]['price'], $product[0]['sale_price']);
				$price += $optionsCost;
				if ($price < 0) $price = 0;
				$name	= $product[0]['name'];
			}
