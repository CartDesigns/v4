...
if ($menucache->cacheStatus) {
	...
	if (!$cache->cacheStatus) {
		...
		buildCatTree($treeData, $treekey = 0);
		$cache->writeCache($treeData);
	}

	if (is_array($treeData)) {
		for ($i = 0, $maxi = count($treeData); $i < $maxi; ++$i) {
		...
		#	echo $treeData[$i]['cat_name'].", cat_id=".$treeData[$i]['cat_id'] .", noProducts=".$treeData[$i]['noProducts'] .", cat_father_id=".$treeData[$i]['cat_father_id'] .", level=". $treeData[$i]['level']."<hr>";
			...
			if ($treeData[$i]['level'] > $upperlevel) {
				for ($n = 0, $maxn = ($treeData[$i]['level']-$upperlevel); $n < $maxn; ++$n){
	...
	if ($config['saleMode']) {
		$sale_items	= $db->select("SELECT COUNT(`productId`) AS count FROM ".$glob['dbprefix']."CubeCart_inventory WHERE `disabled` = '0' AND `sale_price` > 0");
