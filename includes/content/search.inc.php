...
$sql		= sprintf("SELECT `cat_id`, `cat_father_id`, `cat_name` FROM %sCubeCart_category WHERE `hide` = '0' AND (`cat_desc` != '##HIDDEN##' OR `cat_desc` IS NULL) AND `noProducts` >= 1 GROUP BY `cat_father_id`, `cat_id`", $glob['dbprefix']);
