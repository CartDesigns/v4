...
$result = $db->select('SELECT `name` FROM '.$glob['dbprefix'].'CubeCart_inventory WHERE `productId` = '.(int)$db->mySQLSafe($_GET['productId']));

if (LANG_FOLDER != $config['defaultLang']) {
	$foreignVal = $db->select('SELECT `name` FROM '.$glob['dbprefix'].'CubeCart_inv_lang WHERE `prod_master_id` = '.(int)$db->mySQLSafe($_GET['productId']).' AND `prod_lang` = '.$db->mySQLSafe(LANG_FOLDER));
	if ($foreignVal) {
...
$tellafriend	= new XTemplate('content'.CC_DS.'tellafriend.tpl');
	...
	if($config['floodControl']=='recaptcha') {
		...
		$tellafriend->assign('DISPLAY_RECAPTCHA', $recaptcha);
