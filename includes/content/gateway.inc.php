...
if($_GET['contShop'] && isset($_GET['cart_order_id'])) {
	if (isset($_SESSION['cc_back'])) unset($_SESSION['cc_back']);

	$pastBasket = $db->select(sprintf('SELECT `basket` FROM `%sCubeCart_order_sum` WHERE `cart_order_id` = %s',$glob['dbprefix'],$db->MySQLSafe($_GET['cart_order_id'])));
...
if (!$basket && isset($_GET['cart_order_id']) && isset($_POST['gateway']) && !preg_match('#[^0-9a-z\-\_\(\)\s]+#i', $_POST['gateway'])){
	...
	$db->update($glob['dbprefix'].'CubeCart_order_sum',$setgateand,'cart_order_id = '.$db->MySQLSafe($_GET['cart_order_id']));
	...
	if (!(bool)$display_3ds) {
		$gateway->parse('gateway.cart_true.button');
	}
	$gateway->parse('gateway.cart_true');
}
// build order and display payment methods
elseif(($basket || isset($_GET['cart_order_id'])) && !isset($_POST['gateway'])) {
	...
	*/
	...
	if(!isset($_GET['cart_order_id'])){
		// order inventory
		for($i = 1, $maxi = count($basket['invArray']); $i <= $maxi; ++$i){
		...
		$orderSum['add_1_d'] 	= $basket['delInf']['add_1'];
		$orderSum['add_2_d'] 	= $basket['delInf']['add_2'];
		$orderSum['town_d'] 	= $basket['delInf']['town'];
		$orderSum['county_d'] 	= $basket['delInf']['county'];
		$orderSum['postcode_d'] = $basket['delInf']['postcode'];
	...
	$gateway->assign('VAL_FORM_ACTION','index.php?_g=co&amp;_a=step3&amp;cart_order_id='.$cart_order_id);
	$gateway->assign('VAL_FORM_METHOD','post');
	$gateway->assign('VAL_TARGET','_self');
	$gateway->assign('LANG_CHECKOUT_BTN',$lang['gateway']['continue']);
	$gateway->assign('LANG_PAYMENT_SUMMARY', sprintf($lang['gateway']['payment_summary'], priceFormat($orderSum['prod_total'], true), $cart_order_id));
	...
	$gatewayModules = $db->select("SELECT `folder`, `default`, `module` FROM ".$glob['dbprefix']."CubeCart_Modules WHERE (`module`='gateway' OR `module`='altCheckout') AND status = 1");

	if ($gatewayModules) {

		$gateway->assign('LANG_COMMENTS',$lang['gateway']['your_comments']);

		for ($i = 0, $maxi = count($gatewayModules); $i< $maxi; ++$i){
			...
			if ($gatewayModules[$i]['module'] == 'gateway' && file_exists(CC_ROOT_DIR.CC_DS.'modules'.CC_DS.'gateway'.CC_DS.$gatewayModules[$i]['folder'].CC_DS.'transfer.inc.php')) {
				...
				$gateway->assign('TD_CART_CLASS',cellColor($i, 'tdcartEven', 'tdcartOdd'));

				$module = fetchDbConfig($gatewayModules[$i]['folder']);
				$module['desc'] = ($module['logo']) ? '<img src="modules/gateway/'.$gatewayModules[$i]['folder'].'/admin/logo.gif" alt="'.$module['desc'].'" />' : $module['desc'];
				$module['desc'] .= (!empty($module['more_link'])) ? '<br />(<a href="" onclick="openPopUp(\''.$module['more_link'].'\',\'more_info\',550,500,1);" class="txtLink" />Find out more</a>)' : '';
		...
		if(isset($_GET['cart_order_id'])){
			...
			$query = 'SELECT `customer_comments` FROM '.$glob['dbprefix'].'CubeCart_order_sum WHERE `cart_order_id` = '.$db->MySQLSafe($_GET['cart_order_id']);
	...
	$query = "SELECT `folder`, `default` FROM ".$glob['dbprefix']."CubeCart_Modules WHERE module='affiliate' AND `status` = 1";
	$affiliateModule = $db->select($query);

	if ($affiliateModule){

		for($i = 0, $maxi = count($affiliateModule); $i < $maxi; ++$i){
	...
	if(!isset($_GET['cart_order_id'])) {
		httpredir('index.php?_g=co&_a=step3&cart_order_id='.$cart_order_id);
	}
