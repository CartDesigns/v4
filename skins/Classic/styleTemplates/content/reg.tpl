...
{ICON_REQUIRED}</td>
	  ...
	  <tr>
		...
		<div id="divCountySelect" {VAL_COUNTY_SEL_STYLE}>
		  ...
		  <input name="which_field" type="hidden" id="which_field" value="{VAL_COUNTY_WHICH_FIELD}" /> {ICON_REQUIRED}
		</div>
	  ...
	  <tr>
		...
		<td nowrap="nowrap"><input name="password" type="password" autocomplete="off" class="{VAL_PASS_CLASS}" id="password" size="16" value="{VAL_PASSWORD}" tabindex="14" /> 
		  <span id="password_required" {VAL_PASS_HIDE_REQUIRED}>{ICON_REQUIRED_SECURITY}</span> </td>
		<td>{LANG_CONFIRM_PASSWORD}</td>
		<td nowrap="nowrap"><input name="passwordConf" type="password" autocomplete="off" class="{VAL_PASS_CLASS}" id="passwordConf" size="16" value="{VAL_PASSWORD_CONF}" tabindex="15" /> 
