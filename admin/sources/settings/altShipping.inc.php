...
{
	$delete = $db->delete($glob['dbprefix'].'CubeCart_alt_shipping_prices','`alt_ship_id` = '.$db->MySQLSafe($_GET['delete']));
	$delete = $db->delete($glob['dbprefix'].'CubeCart_alt_shipping','`id` ='.$db->MySQLSafe($_GET['delete']));
...
elseif(isset($_GET['dir']))
{
	...
	if($_GET['dir']=='up')
	{
		$query = 'UPDATE '.$glob['dbprefix'].'CubeCart_alt_shipping SET `order` = `order` - 1 WHERE `id` = '.$_GET['id'];
		$db->misc($query);
		$query = 'UPDATE '.$glob['dbprefix'].'CubeCart_alt_shipping SET `order` = `order` + 1 WHERE `id` = '.$_GET['affected'];
		$db->misc($query);
	}
	elseif($_GET['dir']=='dn')
	{
...
elseif(isset($_POST['submit']))
{
	...
	if($_POST['id']>0)
	{
		$update = $db->update($glob['dbprefix'].'CubeCart_alt_shipping',$data,'`id`='.$db->MySQLSafe($_POST['id']));
	...
	{
		$insert = $db->insert($glob['dbprefix'].'CubeCart_alt_shipping',$data);
...
<?php
// repeat region
$query = 'SELECT * FROM '.$glob['dbprefix'].'CubeCart_alt_shipping ORDER BY `order` ASC';
// query database
...
if($results)
{
	...
	<tr>
		...
		<td align="center" class="<?php echo $cellColor; ?>">
		<a <?php if(permission('settings','edit')){ ?>href="<?php echo $global['adminFile']; ?>?_g=settings/altShipping&amp;edit=<?php echo $results[$i]['id']; ?>" class="txtLink"<?php } else {  echo $link401; } ?>><?php echo $lang['admin_common']['edit']; ?></a>		</td>
	    <td align="center" class="<?php echo $cellColor; ?>"><a <?php if(permission('settings','delete')){ ?>href="<?php echo $glob['adminFile']; ?>?_g=settings/altShipping&amp;delete=<?php echo $results[$i]['id']; ?>" onclick="return confirm('<?php echo str_replace("\n", '\n', addslashes($lang['admin_common']['delete_q'])); ?>')" class="txtLink"<?php } else {  echo $link401; } ?>><?php echo $lang['admin_common']['delete']; ?></a></td>
	    <td align="center" class="<?php echo $cellColor; ?>"><a <?php if(permission('settings','read')){ ?>href="<?php echo $global['adminFile']; ?>?_g=settings/altShippingPrices&amp;id=<?php echo $results[$i]['id']; ?>" class="txtLink"<?php } else {  echo $link401; } ?>><?php echo $lang['admin']['shipping_set_prices']; ?></a></td>
	</tr>
	<?php
	}
}
...
if((isset($_GET['edit']) && (int)$_GET['edit']>0) || $_GET['mode']=='new')
{
	...
	{
		$editShip = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_alt_shipping WHERE `id` = '.$db->MySQLSafe((int)$_GET['edit']));
	}
?>
<tr>
	<td>
	...
	<td colspan="3">
	  ...
	  <input type="submit" name="submit" value="<?php if($editShip) { echo $lang['admin_common']['edit']; } else { echo $lang['admin_common']['add']; } ?> " class="submit" />	</td>
</tr>
<?php
}
?>
