...
if(isset($_POST['rates'])){
	...
	foreach ($ratesArray as $rates){
		...
		$res = $db->update($glob['dbprefix'].'CubeCart_tax_rates', $record, $where);
		if ($res) {
			++$update;
		}
	}
...
elseif(isset($_POST['new_rates'])) {
	...
	if($add) {
		if($rates['county_id']==0) {
			$check_add = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_tax_rates WHERE `type_id`='.$record['type_id'].' AND `details_id`='.$record['details_id'].' AND `country_id`='.$record['country_id']);
		} else {
			$check_add = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_tax_rates WHERE `type_id`='.$record['type_id'].' AND `details_id`='.$record['details_id'].' AND `country_id`='.$record['country_id'].' AND (county_id='.$record['county_id'].' OR county_id='.$db->mySQLSafe('0').')');
		}
		if($check_add) {
			$error = true;
		}
		if(!$error){
			$insert = $db->insert($glob['dbprefix'].'CubeCart_tax_rates', $record);
		}
	}

	if ($edit) {
		if ($rates['county_id']==0) {
			$check_edit = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_tax_rates WHERE `type_id`='.$record['type_id'].' AND `details_id`='.$record['details_id'].' AND `country_id`='.$record['country_id'].' AND id <> '.$record['id']);
		} else {
			$check_edit = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_tax_rates WHERE `type_id`='.$record['type_id'].' AND `details_id`='.$record['details_id'].' AND `country_id`='.$record['country_id'].' AND (county_id='.$record['county_id'].' OR county_id='.$db->mySQLSafe('0').') AND id <> '.$record['id']);
		}
		if($check_edit) {
			$error = true;
		}
		if(!$error) {
			$where = '`id` = '.$record['id'];
			$update = $db->update($glob['dbprefix'].'CubeCart_tax_rates', $record, $where);
		}
	}
...
elseif(isset($_GET['delete_rates'])) {
	...
	$delete = $db->delete($glob['dbprefix'].'CubeCart_tax_rates', $where, '');
	if($delete) {
...
$all_states = $db->select('SELECT `id`, `countryId`, `abbrev`, `name` FROM '.$glob['dbprefix'].'CubeCart_iso_counties');
...
$last_id = 0;
if (is_array($taxRates)) {
	for ($i = 0, $maxi = count($taxRates); $i < $maxi; ++$i) {
...
<?php
		$c = 0;
		for ($j = 0, $maxj = count($taxRates); $j < $maxj; ++$j)
		{
			$rates = $taxRates[$j];
			$id = $rates['id'];
...
?>	  </td>
	  <td align="center" class="<?php echo $cellColor; ?> copyText">
	  <a <?php if(permission('settings','delete')){ ?>href="<?php echo $currentURL; echo (strstr($currentURL,'?') ? '&' : '?'); ?>edit=<?php echo $id;?>" class="txtLink" <?php } else { echo $link401; } ?>><?php echo $lang['admin_common']['edit']; ?></a></td>
	  <td align="center" class="<?php echo $cellColor; ?> copyText"><a <?php if(permission('settings','delete')){ ?>href="<?php echo $currentURL; echo (strstr($currentURL,"?") ? "&" : "?"); ?>delete_rates=<?php echo $id; ?>" onclick="return confirm('<?php echo str_replace("\n", '\n', addslashes($lang['admin_common']['delete_q'])); ?>')" class="txtLink"<?php } else { echo $link401; } ?>><?php echo $lang['admin_common']['delete'];?></a></td>
	</tr>
<?php
		}
	}
