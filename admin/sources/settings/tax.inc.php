...
elseif(isset($_POST['details'])) {
	...
	foreach ($detailsArray as $details) {
		...
		$check = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_tax_details WHERE `name`='.$record['name'].' AND `id`<>'.$db->mySQLSafe($details['id']));
		if ($check) {
			++$error;
		} else {
			$res = $db->update($glob['dbprefix']."CubeCart_tax_details", $record, $where);
			if ($res) {
				++$update;
			}
		}
...
elseif(isset($_POST['new_details'])) {
	...
	$check = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_tax_details WHERE `name`='.$record['name']);
	if($check) {
		$insert = false;
	} else {
		$insert = $db->insert($glob['dbprefix']."CubeCart_tax_details", $record);
	}
...
elseif(isset($_GET['delete_details'])) {
	...
	$delete = $db->delete($glob['dbprefix'].'CubeCart_tax_details', $where, '');
	if($delete){
...
if ($config_tax_mod['status']) {
	$taxDetails = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_tax_details ORDER BY `id`');
	...
	$country_names = array();
	$all_countries = $db->select('SELECT `id`, `iso`, `printable_name` FROM '.$glob['dbprefix'].'CubeCart_iso_countries');
	...
	$defaultCountry = $db->select("SELECT id FROM ".$glob['dbprefix']."CubeCart_iso_countries WHERE iso='US'");
...
?>
<form name="updateSettings" method="post" enctype="multipart/form-data" target="_self" action="<?php echo $config['adminFile']; ?>?_g=settings/tax">
<table border="0" cellspacing="1" cellpadding="3" class="mainTable">
	<tr>
	...
	<tr>
	  ...
	  <td align="left">
	  <?php
		$counties = $db->select("SELECT * FROM ".$glob['dbprefix']."CubeCart_iso_counties WHERE `countryId` = '".$config['taxCountry']."' ORDER BY `countryId`, `name` ASC");
	  ?>
	  ...
	  <?php for($i = 0, $maxi = count($counties); $i < $maxi; ++$i){ ?>
...
$taxTypes = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_taxes ');
?>
<form name="taxTypes" method="post" enctype="multipart/form-data" action="<?php echo $config['adminFile']; ?>?_g=settings/tax">
<table border="0" cellspacing="1" cellpadding="3" class="mainTable">
	<tr>
...
?><td class="tdTitle"><strong><?php echo $lang['admin']['settings_rate_per'];?></strong></td><?php } ?>
	...
	<?php if($taxTypes){  for($i = 0, $maxi = count($taxTypes); $i < $maxi; ++$i){ ?>
			<tr>
...
?>
			  <td align="center"><a <?php if(permission('settings','edit')){?>href="<?php echo $glob['adminFile']; ?>?_g=settings/tax&amp;edit=<?php echo $taxTypes[$i]['id']; ?>" class="txtLink"<?php } else { echo $link401; } ?>><?php echo $lang['admin_common']['edit']; ?></a></td>
			  <td align="center">
			  <?php
			  // check for product dependancies
			  $noProducts = $db->select('SELECT `productId` FROM '.$glob['dbprefix'].'CubeCart_inventory WHERE `taxType` = '.$taxTypes[$i]['id']);
			  $noRates = false;
			  if ($config_tax_mod['status']) {
				  $noRates = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_tax_rates WHERE `type_id` = '.$taxTypes[$i]['id']);
			  }

			  if(!$noProducts && !$noRates){
			  ?>
			  <a <?php if(permission('settings','delete')){?>href="<?php echo $glob['adminFile']; ?>?_g=settings/tax&amp;delete=<?php echo $taxTypes[$i]['id']; ?>" onclick="return confirm('<?php echo str_replace("\n", '\n', addslashes($lang['admin_common']['delete_q'])); ?>')" class="txtLink"<?php } else { echo $link401; } ?>><?php echo $lang['admin_common']['delete'];?></a>
			  <?php
			  } else {
			  ?>
	...
	if(isset($_GET['edit']) && (int)$_GET['edit']>0){
		...
		$editTax = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_taxes WHERE `id` = '.$db->mySQLsafe((int)$_GET['edit']));
...
?>
		<td colspan="2" align="center"><input name="submit" type="submit" class="submit" id="submit" value="<?php if(isset($editTax) && $editTax){ echo $lang['admin_common']['edit']; } else{ echo $lang['admin_common']['add']; } ?> <?php echo $lang['admin']['settings_tax'];?>" /></td>
	</tr>
</table>
...
<?php
		for ($i = 0, $maxi = count($taxDetails); $i < $maxi; ++$i)
		{
...
<?php
			...
			$noRates = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_tax_rates WHERE `details_id`='.$id);
			if(!$noRates && permission('settings','delete'))
			{
?>
