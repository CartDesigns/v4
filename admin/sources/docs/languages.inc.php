...
if(!isset($_GET['mode'])){
	...
	if(isset($_GET['edit']) && (int)$_GET['edit']>0){
		$query = sprintf("SELECT * FROM ".$glob['dbprefix']."CubeCart_docs_lang WHERE id = %s", $db->mySQLSafe((int)$_GET['edit']));
	} else {
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <?php if(!isset($_GET['mode'])){ ?><td align="right" valign="middle"><a <?php if(permission('documents','write')){?>href="<?php echo $glob['adminFile']; ?>?_g=docs/languages&amp;mode=new&amp;doc_master_id=<?php echo $_GET['doc_master_id']; ?>" class="txtLink"<?php } else { echo $link401; } ?>><img src="<?php echo $glob['adminFolder']; ?>/images/buttons/new.gif" alt="" hspace="4" border="0" title="" /><?php echo $lang['admin_common']['add_new']; ?></a></td><?php } ?>
  </tr>
</table>
<?php
...
<table width="100%"  border="0" cellspacing="1" cellpadding="3" class="mainTable">
  ...
  <tr>
    ...
    <td align="center" width="10%" class="<?php echo $cellColor; ?>"><a <?php if(permission('documents','edit')){ ?>href="<?php echo $glob['adminFile']; ?>?_g=docs/languages&amp;edit=<?php echo $results[$i]['id']; ?>&amp;doc_master_id=<?php echo $_GET['doc_master_id']; ?>" class="txtLink"<?php } else { echo $link401; } ?> ><?php echo $lang['admin_common']['edit']; ?></a></td>
    <td align="center" width="5%" class="<?php echo $cellColor; ?>"><a <?php if(permission('documents','delete')){ ?>href="<?php echo $glob['adminFile']; ?>?_g=docs/languages&amp;delete=<?php echo $results[$i]['id']; ?>&amp;doc_master_id=<?php echo $_GET['doc_master_id']; ?>" onclick="return confirm('<?php echo str_replace("\n", '\n', addslashes($lang['admin_common']['delete_q'])); ?>')" class="txtLink" <?php } else { echo $link401; } ?>><?php echo $lang['admin_common']['delete']; ?></a></td>
  </tr>
  <?php } // end loop
  } else { ?>
   <tr>
