...
<p class='pageTitle'><?php echo $lang['admin']['stats_monthly_sales'];?></p>
<?php

$query = 'SELECT min(`cart_order_id`) AS `lowest` FROM `'.$glob['dbprefix'].'CubeCart_order_sum` WHERE `status` = 3;';
...
if(isset($_GET['yStart']) && !empty($_GET['yStart'])) {
	...
	$oneYrPrev = strftime('%y%m',mktime( 12 , 0 , 0, $monthNow, 5 , $yearNow)-28927182);
	$oneYrOn = $yearNow.$monthNow;
}

$query = "SELECT `prod_total`, `cart_order_id` FROM `".$glob['dbprefix']."CubeCart_order_sum` WHERE cart_order_id>'".$oneYrPrev."01-000000-0000' AND `cart_order_id`<'".$oneYrOn ."31-999999-9999' AND `status` = 3 ORDER BY `cart_order_id` ASC;";
...
if($year) {
	for($i = 0, $maxi = count($year); $i < $maxi; ++$i){
