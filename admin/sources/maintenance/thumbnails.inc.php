...
if (isset($_POST['rebuild'])) {
	...
	if (is_array($_POST['thumbName'])) {
		...
		foreach ($_POST['thumbName'] as $thumbName){
			$thumbName = filter_var($thumbName, FILTER_SANITIZE_STRING);
...
<?php
	...
	if (is_array($dirArray)) {
		foreach($dirArray as $file) {
			...
			if (checkImgExt(strtolower($file)) && (strstr($file, "thumb_")) === false){
				++$i;
				$cellColor = '';
...
?>
	  <tr>
		...
		<a href="javascript:;" onclick="openPopUp('<?php echo $glob['adminFile'];?>?_g=filemanager/preview&amp;file=<?php echo $masterfile_rootRel."&amp;x=".$size[0]."&amp;y=".$size[1]; ?>','filemanager',<?php echo $size[0]+12; ?>,<?php echo $size[1]+12; ?>)" class="txtDir"><img src="<?php echo $thumb_rootRel; ?>" border="0" alt="" /></a>
		<?php
