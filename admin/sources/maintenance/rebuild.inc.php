...
#	}
	...
	$sql = 'SELECT * FROM '.$glob['dbprefix'].'CubeCart_cats_idx ORDER BY `productId`, `cat_id` ASC';
	...
	if($results) {
		...
		if($flagged) {
			foreach($results as $product) {
				if($product['flag']) {
					$db->delete($glob['dbprefix'].'CubeCart_cats_idx', '`id` = '.$db->MySQLSafe($product['id']));
				}
			} // end foreach loop
	...
	$update = $db->update($glob['dbprefix'].'CubeCart_customer', $record);
...
<table border="0" cellspacing="1" cellpadding="3" class="mainTable">
  ...
  <tr>
	...
	<td class="tdText"><input name="button" type="button" value="<?php echo $lang['admin_common']['update'];?>" class="submit" onclick="goToURL('parent','<?php echo $glob['adminFile']; ?>?_g=maintenance/rebuild&amp;filemanager=1');return document.returnValue" /></td>
  </tr>
  <tr>
