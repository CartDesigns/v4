...
if(isset($_GET['delete']) && $_GET['delete']>0) {
	...
	httpredir($glob['adminFile']."?_g=categories/languages&cat_master_id=".$_GET['cat_master_id']);
...
if(!isset($_GET['mode'])) {
	...
	if(isset($_GET['edit']) && (int)$_GET['edit']>0) {
		$query = sprintf('SELECT * FROM '.$glob['dbprefix'].'CubeCart_cats_lang WHERE `id` = %s', $db->mySQLSafe((int)$_GET['edit']));
	} else {
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <?php if(!isset($_GET['mode'])){ ?><td align="right" valign="middle"><a <?php if(permission('categories','write')){?>href="<?php echo $glob['adminFile']; ?>?_g=categories/languages&amp;mode=new&amp;cat_master_id=<?php echo $_GET['cat_master_id']; ?>" class="txtLink"<?php } else { echo $link401; } ?>><img src="<?php echo $glob['adminFolder']; ?>/images/buttons/new.gif" alt="" hspace="4" border="0" title="" /><?php echo $lang['admin_common']['add_new']; ?></a></td><?php } ?>
  </tr>
</table>
<?php
if((isset($_GET['edit']) && (int)$_GET['edit']>0) || (isset($_GET['mode']) && $_GET['mode']=='new')){
...
<form action="<?php echo $glob['adminFile']; ?>?_g=categories/languages&amp;cat_master_id=<?php echo $_GET['cat_master_id']; ?>" target="_self" method="post" language="javascript">
...
<table width="100%"  border="0" cellspacing="1" cellpadding="3" class="mainTable">
  ...
  <tr>
    ...
    <td align="center" width="10%" class="<?php echo $cellColor; ?>"><a <?php if(permission('categories','edit')){ ?>href="<?php echo $glob['adminFile']; ?>?_g=categories/languages&amp;edit=<?php echo $results[$i]['id']; ?>&amp;cat_master_id=<?php echo $_GET['cat_master_id']; ?>" class="txtLink"<?php } else { echo $link401; } ?> ><?php echo $lang['admin_common']['edit']; ?></a></td>
    <td align="center" width="5%" class="<?php echo $cellColor; ?>"><a <?php if(permission('categories','delete')){ ?>href="<?php echo $glob['adminFile']; ?>?_g=categories/languages&amp;delete=<?php echo $results[$i]['id']; ?>&amp;cat_master_id=<?php echo $_GET['cat_master_id']; ?>" onclick="return confirm('<?php echo str_replace("\n", '\n', addslashes($lang['admin_common']['delete_q'])); ?>')" class="txtLink" <?php } else { echo $link401; } ?>><?php echo $lang['admin_common']['delete']; ?></a></td>
  </tr>
  <?php } // end loop
  } else { ?>
   <tr>
