...
if (isset($_POST['ProcessImport']) && file_exists(CC_ROOT_DIR.CC_DS.'cache'.CC_DS.'importdata')) {
	...
	$catSearch = sprintf("SELECT * FROM %sCubeCart_category WHERE `cat_name` = 'Imported Products'", $glob['dbprefix']);
	...
	if ($result) {
		...
		$db->insert($glob['dbprefix'].'CubeCart_category', $record);
		## Define the cat_id
		$cat_id = $db->insertid();
	}

	if ($_POST['ImportMethod'] == 'replace') {
		$db->misc('TRUNCATE TABLE '.$glob['dbprefix'].'CubeCart_inventory;');
		$db->misc('TRUNCATE TABLE '.$glob['dbprefix'].'CubeCart_cats_idx;');
	}
	...
	while (($data = fgetcsv($fp, 5000, $delimiter)) !== false) {
		if ($i==0 && $_POST['ColumnHeaders']=='1') {
			...
			$query = sprintf('INSERT INTO %sCubeCart_inventory (%s) VALUES (%s);', $glob['dbprefix'], implode(',', $fields), implode(',', $values));
			$db->misc($query);

			## Add index to cats_idx table
			$query = sprintf("INSERT INTO %sCubeCart_cats_idx (`cat_id`,`productId`) VALUES ('%d', '%d');",$glob['dbprefix'], $current_cat_id, $db->insertid());
			$db->misc($query);
