...
if(isset($_POST) && isset($_GET['delete']) && $_GET['delete']>0){
	...
	if($_POST['id']>0){
		...
		// update
		$result = $db->update($glob['dbprefix'].'CubeCart_Coupons', $data, '`id` = '.$db->mySQLSafe($_POST['id']));
	...
	// insert
		...
		$query = 'SELECT `id` FROM '.$glob['dbprefix'].'CubeCart_Coupons WHERE `code` = '.$db->mySQLSafe($_POST['code']);
		$exists = $db->select($query);

		if($exists) {
			$msg = "<p class='warnText'>".sprintf($lang['admin']['coupon_already_exists'],$_POST['code'])."</p>";
		} else {
			$result = $db->insert($glob['dbprefix']."CubeCart_Coupons", $data);
		}
...
$query = 'SELECT * FROM '.$glob['dbprefix'].'CubeCart_Coupons ORDER BY `code` ASC';
...
<table width="100%"  border="0" cellspacing="1" cellpadding="3" class="mainTable">
  ...
  <tr>
    ...
    <td align="center" class="<?php echo $cellColor; ?>">
	<a <?php if(permission('offers','edit')){ ?>href="<?php echo $glob['adminFile']; ?>?_g=products/coupons&amp;edit=<?php echo $results[$i]['id']; ?>" class="txtLink"<?php } else { echo $link401; } ?>><?php echo $lang['admin_common']['edit'];?></a>	</td>
    <td align="center" class="<?php echo $cellColor; ?>">
	<a <?php if(permission('offers','delete')){ ?>href="<?php echo $glob['adminFile']; ?>?_g=products/coupons&amp;delete=<?php echo $results[$i]['id']; ?>" onclick="return confirm('<?php echo str_replace("\n", '\n', addslashes($lang['admin_common']['delete_q'])); ?>')" class="txtLink"<?php } else { echo $link401; } ?>><?php echo $lang['admin_common']['delete'];?></a>	</td>
    <td align="center" class="<?php echo $cellColor; ?>">

	<?php if($results[$i]['status']==1){ ?>
	<a <?php if(permission('offers','edit')){ ?>href="<?php echo $glob['adminFile']; ?>?_g=products/coupons&amp;id=<?php echo $results[$i]['id']; ?>&amp;status=0" class="txtRed"<?php } else { echo $link401; } ?>><?php echo $lang['admin_common']['disable'];?></a>
	<?php } else { ?>
	<a <?php if(permission('offers','edit')){ ?>href="<?php echo $glob['adminFile']; ?>?_g=products/coupons&amp;id=<?php echo $results[$i]['id']; ?>&amp;status=1" class="txtGreen"<?php } else { echo $link401; } ?>><?php echo $lang['admin_common']['enable'];?></a>
	<?php } ?>	</td>
  </tr>
...
<br />
<?php
if(permission('offers','write') && permission('offers','edit')){

	if((int)$_GET['edit']>0) {
		$query = 'SELECT * FROM '.$glob['dbprefix'].'CubeCart_Coupons WHERE `id` ='.$db->mySQLSafe((int)$_GET['edit']);
